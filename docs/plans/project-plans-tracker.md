...
| 日期 | 类型 | 文档路径 | 说明 |
|------|------|----------|------|
| 2026-09-18 | spec | docs/specs/2026-09-18-Spec-PoolStopOptimization-v1.0.md | 代理池关闭加速（30-120s → 1-2s）— 用户报告"代理池中代理数量多时关闭池非常缓慢"；根因=三瓶颈耦合：B1 StandaloneProxyPool::stop join evaluatorThread_（主因，30-120s 与成员数正相关）+ B2 ProxyHealthEvaluator::doProbe 串行遍历所有 ACTIVE 成员（cURL 探测平均 ~1s/成员）+ B3 ProxyProbePool::stop 串行停止 worker（probeWorkers=8 时 ~12s）+ B4 XrayInstance::stop 硬 sleep(500ms)+WaitForSingleObject(500ms)（单实例 1.0-1.5s）；方案 E=A+B+C 组合（不采用 UI 异步 stop 因历史 UAF 教训）：A=ProxyHealthEvaluator::probe 增加 std::atomic&lt;bool&gt;* stopFlag 参数让 doProbe 可中断（循环头检查 + 尾部补占位保 API 契约），B=XrayInstance::stop 用 20ms poll 循环替代硬 sleep(500ms)（上限仍 500ms），C=ProxyProbePool::stop 用 std::async 并行停止 worker（wait_for 3s 守护）；TDD：新增 4 测试（A 离线 1ms PASSED + B/C/D opt-in XRAY_REAL_EXE，本环境 B/C PASSED、D 因缺 geoip.dat GTEST_SKIP）；test_standalone_proxy_pool 11 passed + 7 skipped + 1 pre-existing failed（ResolvePoolPortsKeepsFreeDesiredPort 端口冲突，修改前就失败）；C++17 无 auto | ✅ completed |
| 2026-09-17 | plan | docs/plans/2026-09-17-Plan-HealthMonitorUnify-v1.0.md | 方案 B 配置层聚合+控制层解耦 — independent_probe.enabled 独立开关解耦 silent 探活（默认 true 兼容旧配置）；MainFrame timer 生命周期改 `monitor.enabled \|\| probe.enabled` 驱动、timer 回调内探活单独门控；ConfigDialog 按 2026-09-17 评审层级树重组（代理后端选择条件可见/监控悬浮窗/健康度评估父分类+独立代理评估+代理池评估/代理池配置）；暴露 scoring_*_weight 三权重（补 serializer 缺口）；悬浮窗新增「独立代理探活」勾选框；验证=构建 0 error + ctest 46/47（唯一失败 UI_FLOATINGWIDGET Orb hit-test 为既有环境 flake，HEAD 基线同样失败） | ⏳ executing |
| 2026-09-17 | spec | docs/specs/2026-09-17-Spec-TestMessageOnlyValidProxy-Fix-v1.0.md | 修复测试/批量测试时仅对有效代理刷新 `ProfileExItem.message` 测试时间侧 — `updateTestResult` L274 与 `updateTestResultBatch` L404 原用 `if (success)` 判断，导致 `success=true 但 latencyMs<=0` 的代理也被写入测试时间；修复=两处改为 `if (utils::isTestResultValid(success, latencyMs))`（`success && latencyMs > 0`），与 `delayStr` 计算口径一致；`!success` 分支嵌套在 else 内，仅失败时重置评价历史列；C++17 无 auto | ✅ completed |
| 2026-09-17 | spec | docs/specs/2026-09-17-Spec-ProxyPoolInjectMessageUpdate-Fix-v1.0.md | 修复加入代理池未更新 `ProfileExItem.message` 启动时间侧 — `AppController::injectProxyToPool` 调 `pool->injectMember` 成功后未调用 `exDao_.updateStartupTime(indexId, db_)`；对比 `startStandaloneProxy` L1162、`adoptDanglingStandaloneProxies` L1766 均有调用；修复=`injectMember` 成功返回后追加 `exDao_.updateStartupTime(indexId, db_)`，与独立代理启动/纳管语义对齐；C++17 无 auto | ✅ completed |
| 2026-09-17 | spec | docs/specs/2026-09-17-Spec-ProxyMonitorMainFrameConfigSync-Fix-v1.0.md | 修复 `onMenuConfig` 热应用后 MainFrame::config_ 未同步致「监控代理」按钮误报「监控代理进程未在配置中启用」— `onMenuConfig` L1178 `controller_->saveConfig(cfg)` 只更新 AppController::config_，MainFrame::config_ 仍为旧值；`onMenuStandaloneMonitor` L1121 用 MainFrame::config_.proxy_process_monitor.enabled 判断，导致用户在 GUI 配置窗口将 enabled 改为 true 并保存后单击「监控代理」按钮弹出误报提示；修复=saveOk 检查之后追加 `config_ = cfg;` 一行同步；C++17 无 auto | ✅ completed |
| 2026-09-17 | spec | docs/specs/2026-09-17-Spec-ProxyMonitorStatusBarRefresh-Fix-v1.0.md | 修复 `proxy_process_monitor.enabled=true` 时状态栏代理进程监控不刷新（仍显示灰色）— `MainFrame::startMonitoring` 中 `repositionProxyMonPanel()` 从 `startProxyMonitor` 之后提前到之前，保证 panel 拿到最终 `GetFieldRect(3)` 尺寸后再 `updateProxyMonStatus(true, 0)` Refresh（原顺序下首次 paint 可能面对 0×0 client size 触发 paint handler L499 早退）；末尾追加 `Refresh()` 兜底防 wxWidgets paint 事件合并；paint handler / startProxyMonitor / repositionProxyMonPanel 三处新增 DEBUG 日志便于事后验证；C++17 无 auto | ✅ completed |
| 2026-09-17 | spec | docs/specs/2026-09-17-Spec-OnlineProxiesTestReportLog-FailedHosts-v1.0.md | Online proxies test 汇总 REPORT 日志补 failed_hosts 明细 — doTestOnlineProxies 主循环外新增 std::string failedHostsStr 累加器，两个失败分支（socks port unknown / !r.success）在 failedIndexIds.push_back 之后追加 host:port（分隔符 " | " 沿用 UnifiedMonitorRows 范式），failed>0 的 REPORT 拼接追加 ", failed_hosts=" 字段；failed==0 两条分支保持简洁；零新增 DAO 查询、零新增配置；C++17 无 auto | ✅ completed |
| 2026-09-16 | spec+plan | docs/specs/2026-09-16-Spec-ProbeNonBlocking-v1.0.md + docs/plans/2026-09-16-Plan-ProbeNonBlocking-Phase1-v1.0.md | 探活不干扰正常操作阶段1 — silent 探活改独立 probeThread_（不占 isRunning_，自动任务/配置保存不再被探活拒）；探活完成 OnlineProbeFinishedEvent 携带受测 indexId → 增量刷新（`refreshResultsFor`，不再全量 53k 重读，UI 不迟滞）；手动测试优先取消探活；DB 切换需探活空闲 + `exDao_.setDb` 重绑防切库后旧句柄 UAF（§8.8）；增量路径 `syncHistoryForIndexId` 单行重算历史三列防探活失败后 Health 陈旧（§8.9）；test_proxy_list_model 27/27 + test_config_reader 43/43；提交 66f9635..4c646e9 | ✅ completed |
