...
## Issue: Export Share Link Count Error - Root Cause Analysis
- **Symptom**: The number of exported valid proxy share links is incorrect
- **Root cause identification**:
  1. **parseSubscription() returns all parsed profiles** (including duplicates and disabled ones)
  2. **updateProfileItems() inserts ALL parsed profiles** into database without filtering
  3. **ProxyFinder::findFirstWorkingProxy() fetches fresh proxies** via network calls
  4. **Mismatch occurs** because:
     - parseSubscription() may parse N profiles from subscription
     - But some may be duplicates, disabled, or fail validation
     - ProxyFinder tests actual connectivity, different from parsing logic
     - Export happens BEFORE deduplication in some code paths
- **Key problematic code locations**:
  - SubitemUpdaterV2::run() line 111-172: Direct update path without proper filtering
  - SubitemUpdaterV2::updateWithStrategy() line 275-308: Strategy-based update
  - Missing validation: No check if parsed profiles are already in database
  - Race condition: Fresh proxy test may find different proxies than parsed ones
- **Debug steps**:
  1. Add logging in parseSubscription() to count valid profiles vs parsed lines
  2. Add logging in updateProfileItems() to count inserted vs skipped profiles
  3. Add logging in ProxyFinder to count successful vs failed proxy tests
  4. Compare counts: parsed vs inserted vs exported
  5. Check database state before and after export
  6. Verify deduplicate() runs BEFORE export in all code paths
- **Test cases for verification**:
  1. Single subscription with N unique valid proxies (count should match N)
  2. Subscription with M duplicate proxies (count should be unique count)
  3. Subscription with K disabled/invalid proxies (count should exclude these)
  4. Multiple subscriptions with overlapping proxies (deduplication test)
  5. Verify export count == database query count for same subscription
- **Fix approach**:
  1. Ensure deduplicate() runs BEFORE export in ALL code paths
  2. Add validation to skip already-imported profiles
  3. Add logging at each stage to trace count discrepancies
  4. Consider atomic operation: parse -> validate -> deduplicate -> export

## Long-Term Memory
- **Project**: C++ Proxy Validation Tool for v2rayn/Xray-core
- **Database Path**: E:/v2rayN-windows-64/guiConfigs/guiNDB.db
- **Reference Context**: v2rayn (C#) and Xray-core (Go/C++)
- **Build System**: CMake 3.16+ with Ninja, targeting Windows with MinGW/GCC
- **Dependencies**: Boost.JSON, SQLite3, cURL, Threads
- **Code Standards**: PascalCase classes, camelCase methods, snake_case members
- **Testing**: Google Test framework for unit/integration/performance tests
- **Key Modules**: XrayManager (singleton), ProxyFinder, ConfigGenerator, ProxyBatchTester
- **CLI Commands**: -c, -show-sub, -G, -F, -FMIN, -U, -UA, -T, -D, -TU, -h
- **Generated Files**: 
  - Log files: log/find_proxy_<timestamp>.log, log/test_log_<timestamp>.log, log/dedup_<timestamp>.log
  - Config files: config/config.json
  - Output files: bin/proxies/proxies_<timestamp>.txt
  - Generated configs: config/<profile_indexid>.json
- **Read Paths**: 
  - Default DB: E:/v2rayN-windows-64/guiConfigs/guiNDB.db
  - Config path: config/config.json (relative to exe dir)
  - Config read via: config::ConfigReader::getDefaultConfigPath()
- **Current Issue**: Export share link count mismatch requires investigation
- **Compilation Environment**: PowerShell + g++ + Ninja
- **Build Toolchain**: CMake 3.16+, Ninja, MinGW/g++ on Windows
- **External Tools**: curl, sqlite3, boost (json, system, filesystem components)