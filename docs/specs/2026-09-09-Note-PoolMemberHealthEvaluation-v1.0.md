...
| 环节 | 位置 | 行为 |
| --- | --- | --- |
| 评估循环 | `StandaloneProxyPool.cpp:296 evaluatorLoop()` | 后台线程每 `cfg_.evaluate.intervalSec`（默认 10s）一轮：`relaunchIfNeeded → doProbe → mergeHealth → 策略 → 两阶段移除 → reportHealth → notifyChanged` |
| 探测装配 | `doProbe()` (:243) | 锁内快照 ACTIVE 成员 → `MemberProbeTarget{tag, configtype=atoi(profile.configtype), address, port, username=profile.security, password=profile.id}`；`testUrl = cfg_.probeUrl 非空 ? cfg_.probeUrl : cfg_.observatory.destination`；`totalMs = timeoutSec*1000`（默认 5000）；`connectMs = min(totalMs, 3000)` |
| 测试 URL 注入 | `AppController.cpp:1794` | `poolCfg.probeUrl = config_.test_url`（复用全局测试 URL，修复 frozen-field health bug） |
| 直连实测 | `ProxyHealthEvaluator.cpp` | 串行 cURL **HEAD** 经上游代理到 testUrl：configtype==4→`socks5h://`；==10→`http://`；`delayMs=getTotalTime()*1000`；`alive=(200<=code<400)`；免 TLS 校验 |
| 结果写回 | `mergeHealth()` | 仅处理 `px-` 前缀且已在 members_ 的成员；alive→failStreak=0，失败→failStreak+=1；`lastProbe=now` |
| 两阶段剔除 | `evaluatorLoop()` | ACTIVE→（autoPruneDead 且 failStreak>=pruneFailStreak(3)）→REMOVE_REQUESTED→下一周期 `removeOutbound` 成功→DRAINING→erase |
| 同步探测 | `StandaloneProxyPool.cpp:208 probeNow()` | `doProbe → mergeHealth → notifyChanged` 单轮入口；**grep 全 src 无任何调用方**（头文件注释声称供注入后即时反馈，实际未接线） |## 3. 已识别问题
...
| P2 | `probeNow()` 无调用方：成员注入后不立即测试，最长等一个评估周期（默认 10s） | 体验与反馈延迟；「加入代理池 → 期望立即看到健康」落空 | 头文件注释声称的即时反馈能力未接线 |
...
- [ ] 接线 `probeNow()`：注入成功 / UI「刷新」后立即触发单轮评估
