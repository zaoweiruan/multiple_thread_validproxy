...
| [Bug 修复记录](#91-bug-修复记录) | 46 | `docs/bugfix/` |
...
| 28 | [`docs/specs/2026-08-13-Spec-ProxyScoring-History-v1.0.md`](./specs/2026-08-13-Spec-ProxyScoring-History-v1.0.md) | **代理评分引入历史服务记录**（v1.2，已吸收 R1-R5 评审修订）— 明细表 `proxy_runtime_history`（一次启动一行，ended_at/exit_code/duration_ms 退出时回填）+ `ProfileExItem` 3 聚合列（start_count/total_runtime_ms/crash_count，写时同步同事务）；`ProcessInspector` 进程枚举+command line 比对（R1 启动前查重重复配置弹窗退出）；**退出检测事件驱动（R5）**：`ProcessExitListener` 每受管进程一个专用等待线程阻塞 `WaitForSingleObject(handle,INFINITE)`（进程句柄即系统级终止通知，零延迟零轮询），finalize 时一次性 command line 检查区分正常退出/崩溃/外部重启接管；R2 启动后 curl 经 socks 端口连通性验证，通过才 `insertStart` 入表，不连通弹窗询问关闭/弃管（弃管=不入表不监控）；`ProxyScorer` 纯函数三因子评分（**速度20%+稳定性30%+历史服务50%**，历史=启动次数 kicker + 贝叶斯平滑正常退出率 + 平均运行时长，冷启动中性 50 分、40:60 归一化不惩罚）；D1 仅 StandaloneProxy / D2 明细+聚合 / D3 ProcessExitListener 已确认 | 🔍 draft |
