...
| 33 | [`docs/specs/2026-08-24-Spec-StandaloneFloatingWidget-v1.0.md`](./specs/2026-08-24-Spec-StandaloneFloatingWidget-v1.0.md) | **可配置悬浮窗监控独立代理进程（StandaloneFloatingWidget）** — 新增无边框置顶悬浮窗，常驻展示受管独立代理进程 Host/运行时长/SOCKS端口/总数；config.json 新增 `floating_widget` 段（enabled/show_on_startup/refresh_interval_ms[500,60000]/opacity_percent[10,100]/always_on_top/hide_when_empty/max_rows[1,20]/pos_x/pos_y）含类型 WARN + 数值钳制；复刻 ProxyProcessMonitorConfigParser header-only 模式 + ConfigJsonSerializer 写回；复刻 getWatchedStandaloneMonitors() 数据链（零新增查询/零 schema 变更）；行单击经 LocateProxyEvent 定位 ProxyListPanel，「详情」按钮经注入回调打开既有 StandaloneMonitorDialog；Proxy 菜单 checkable 项 Ctrl+Shift+M（ID_MENU_FLOATING_WIDGET = wxID_HIGHEST+115）；可见性状态机区分用户意图/空态自动隐藏/×关闭粘性；退出时位置回写 saveConfig；纯逻辑抽 FloatingWidgetPolicy.h（无 wx 依赖）供单测；UIIds.h 固化 + build-and-test.bat UI 冒烟；**当前状态 📝 draft 待评审（含 D1-D5 决策点：无边框 vs 微型标题栏 / 行单击定位语义 / v1.0 仅 config.json 入口 / hide_when_empty 默认 true / 行内停止按钮延后 v1.1）** | 📝 draft |
